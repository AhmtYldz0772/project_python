...
# Düşman yenildiğinde müzik ve ödül yönetimi
def handle_enemy_defeat(enemy_index):
    ...
    if enemy.health <= 0:
        ...
        # Müzik sıralaması
        music.stop()
        music.play_once("oldurme")
        music.queue("oyun_muzigi")
...
def on_key_down(key):
    ...
    if enemy_index != -1:
        ...
        handle_enemy_defeat(enemy_index)
...
def update(dt):
    ...
    if keyboard.space:
        animate(player, tween ="bounce_end", duration = 1, y =player.y  + cell.height )
